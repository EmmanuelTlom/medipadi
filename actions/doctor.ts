...
import { cancelAppointmentSchema, completeAppointmentSchema } from "@/lib/validations/appointment";
 ...
 */
export async function setWeeklyAvailability (formData) {
  ...
  try {
    ...
    const existing = await db.weeklyAvailability.findFirst({
      where: {
        doctorId: doctor.id,
        dayOfWeek,
      },
    ...
    if (existing) {
      // Update existing
      availability = await db.weeklyAvailability.update({
      ...
      // Create new
      availability = await db.weeklyAvailability.create({
        data: {
          doctorId: doctor.id,
          dayOfWeek,
          startTime,
...
export async function getWeeklyAvailability () {
  ...
  try {
    ...
    console.error("Failed to get weekly availability:", error);
 ...
 */
export async function deleteWeeklyAvailability (availabilityId) {
  const { userId } = await auth();
...
/**
 * Set doctor's availability slots (DEPRECATED - keeping for backward compatibility)
 */
...
export async function cancelAppointment (formData: FormData) {
  ...
  try {
    ...
    const validatedData = cancelAppointmentSchema.parse(rawData);
    const { appointmentId } = validatedData;
